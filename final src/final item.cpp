...
void Item::buyItem(std::vector<Item*>& items, std::vector<Bank*>& accounts, std::string itemName, int amount, int accountNumber){
    ...
    if (prize > 0) {
        int availablePrize = prize - itemToBuy->get_amount();
        if(itemToBuy->get_amount() >= prize){
            itemToBuy->set_amount(itemToBuy->get_amount() - prize);
            cout << "Congratulations! You've received " << prize << " extra " << itemName << " as a gift!" << endl;
        }
        else if(availablePrize > 0){
            itemToBuy->set_amount(itemToBuy->get_amount() - availablePrize);
            cout << "Congratulations! You've received " << availablePrize << " extra " << itemName << " as a gift!" << endl;
        }
    }
}